...
function handleMintClick(event) {
  var element_choice = event.target['alt'];
  if (evolveActive) {
...
async function mintNFT(element) {
  ...
  document.getElementById('avatarInfoBottom').textContent = "MINTING"
  var loop_count = 1;
  while ( !transactionReceipt ) {
    ...
    if (loop_count > 3) {
      ...
      document.getElementById('avatarInfoBottom').insertAdjacentText('beforeEnd', '.');
    }
  ...
  document.getElementById('avatarInfoBottom').textContent = "MINT SUCCESS!"
...
async function levelUp() {
  ...
  var transactionReceipt = await provider.getTransactionReceipt(transactionHash);
  console.log("Immediate transaction receipt: ", transactionReceipt);
  document.getElementById('avatarInfo').textContent = "LEVELING"
  var loop_count = 1;
  while ( !transactionReceipt ) {
    await pause(500);
    if (loop_count > 3) {
      ...
      document.getElementById('avatarInfo').insertAdjacentText('beforeEnd', '.');
    }
    transactionReceipt = await provider.getTransactionReceipt(transactionHash);
    loop_count+=1;
  }
  ...
  console.log("Mined transaction receipt: ", transactionReceipt);
  document.getElementById('avatarInfo').textContent = "LEVEL UP!"
  console.log("Transaction Info: ", transactionInfo);
    ...
    <div className='avatarInfoContainer SlideRightAnimation'>
      ...
      <div className='avatarInfoRight'>
        <div id="avatarInfoStats" className="avatarInfoStats">
          ...
          <div id="imageURIText">Image URI: {user_stats['Image URI']}</div>
